...
# ─────────────────────────────────────────────
# SIDEBAR — FILTROS POR SELECCIÓN ÚNICA
# ─────────────────────────────────────────────
st.sidebar.title('🏭 Filtros')

linea_sel = st.sidebar.selectbox(
    '📋 Línea de producción',
    ['Todas'] + sorted(df['linea_produccion'].unique())
)

turno_sel = st.sidebar.selectbox(
    '🕐 Turno',
    ['Todos'] + sorted(df['turno'].unique())
)
...
maquina_sel = st.sidebar.selectbox(
    '⚙️ Máquina',
    ['Todas'] + sorted(df['maquina'].unique())
)
...
dff = df.copy()
if linea_sel != 'Todas':
    dff = dff[dff['linea_produccion'] == linea_sel]
if turno_sel != 'Todos':
    dff = dff[dff['turno'] == turno_sel]
if producto_sel != 'Todos':
    dff = dff[dff['producto'] == producto_sel]
if maquina_sel != 'Todas':
    dff = dff[dff['maquina'] == maquina_sel]
...
st.markdown('**Alberto Garcia** &nbsp;·&nbsp; Análisis de Producción')
st.caption(
    f'Mostrando {len(dff)} órdenes de producción · '
    f'{dff["fecha_produccion"].min().strftime("%d/%m/%Y") if len(dff) else "—"} → '
    f'{dff["fecha_produccion"].max().strftime("%d/%m/%Y") if len(dff) else "—"}'
)
st.markdown('---')
...
def graf_eficiencia_defectos(grupo, titulo_grupo):
    ...
    fig.update_layout(barmode='group', title=f'Eficiencia vs Defectos por {titulo_grupo}',
                      yaxis_title='%', legend=dict(orientation='h', y=1.12), height=380)
    return fig
...
color_by = st.selectbox(
    'Desglosar por:',
    ['(ninguno)', 'linea_produccion', 'turno', 'producto', 'maquina'],
    format_func=lambda x: {
        '(ninguno)': 'Sin desglose', 'linea_produccion': 'Línea',
        'turno': 'Turno', 'producto': 'Producto', 'maquina': 'Máquina'
    }.get(x, x)
)
...
with c3:
    color_scatter = st.selectbox(
        'Color por', ['linea_produccion', 'turno', 'producto', 'maquina'],
...
with col_e2:
    costo_prod = (dff.groupby('producto')['costo_produccion_cop']
                  .mean().reset_index()
                  .sort_values('costo_produccion_cop', ascending=False))
